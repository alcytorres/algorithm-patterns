...
def max_sum_sliding_window(arr, k):
    ...
    # Set the maximum sum to our first group's sum
    # Why? It's the biggest sum we know so far (we just started!)
    max_sum = window_sum 
    
    # Move the window across the array, one step at a time
    # Why? To check every possible group of k numbers
    for i in range(k, len(arr)):
        # Update the sum: add the new number, subtract the old one
        # Why? This slides our group over by one spot efficiently
        window_sum += arr[i] - arr[i - k]  
        
        # Update the maximum if our new sum is bigger
        # Why? We want to keep track of the largest sum we find
        max_sum = max(max_sum, window_sum)
