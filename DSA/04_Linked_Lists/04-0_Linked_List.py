...
Arrays and linked lists both store collections of data, but they work very differently under the hood.
"""
...
"""
============================
SENTINEL NODES — RECAP
============================

🧠 Big Picture
--------------
We add two fake nodes:
  • head (sentinel) — before the real first node
  • tail (sentinel) — after the real last node

They are NOT part of the data. They exist to:
  • avoid edge-case bugs (None checks)
  • make front/back inserts/removals uniform O(1)

Real first = head.next
Real last  = tail.prev
"""
...
💡 Why sentinels help
--------------------
  - No special-case for “deleting last/first” → tail/head catch the links.
  - No None.next explosions — ends always point to a sentinel.
  - Front/back ops become symmetric and O(1).
"""

# ----------------------------
# ⚠️ Common Gotchas (DLL + sentinels)
# ----------------------------
"""
  - Always update FOUR pointers on insert/delete in DLL.
  - Empty-list checks become simple: head.next is tail (empty).
  - Be careful to start traversal at head.next and stop at tail.
"""
...
“Sentinels are bumpers at both ends:
  • head guards the front, tail guards the back,
...
# ----------------------------
# 🔁 Traversal with a dummy pointer
# ----------------------------
"""
Dummy pointer = a temporary cursor you move, so you don't lose your saved references (head/tail).
"""
