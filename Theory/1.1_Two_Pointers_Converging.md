...
Five shapes cover essentially every converging problem you'll meet, and all five appear in this block:
...
| n up to 10⁵ | (10⁵)² = 10 billion (10¹⁰). Too slow. Need O(n) or O(n log n). |
| n up to 3000 (15 3Sum) | 3000² = 9 million (9×10⁶). Allowed. Don't try to beat O(n²). |
...
On a sorted array, **left is the small end, right is the big end.** Moving a pointer always walks it *inward*, so:

> **Left lifts, right reduces.**
>
> `l += 1` walks toward bigger numbers → **sum goes up**.  
> `r -= 1` walks toward smaller numbers → **sum goes down**.

When the current pair is wrong:

```
sum too small  →  need a lift     →  l += 1
sum too big    →  need a reduce   →  r -= 1
```

And the picture to hold in your head:

> Two inspectors start at opposite ends of a line of people. Each round they look at the two people they're standing in front of, then **one inspector walks inward**. The person they just left is done — never checked again. They meet in the middle. Nobody walks the line twice.

That image is doing real work: "the person they left is done" is why it's O(n), and "nobody walks the line twice" is why it's O(1) space.
...
**The one insight:** 3Sum is just **2Sum-on-a-sorted-array, run n times**. Freeze one number `nums[i]`; now you need two more numbers summing to `-nums[i]`, which is exactly problem 167. Sorting is what makes the two-pointer part work, and it's *also* what makes **dedup** easy — skip duplicates so you don't report the same triplet twice. Identical values end up adjacent after sorting.
...
Every converging two-pointer solution rests on one **invariant** — a fact that stays true on every loop:
...
MEMORY HOOK
  Left lifts, right reduces.
  l += 1 → bigger numbers → sum UP     |    r -= 1 → smaller numbers → sum DOWN
  too small → l += 1                   |    too big → r -= 1
  Two inspectors on a line of people; one walks inward each round. The person they left is done.
