...
| **LC 26 / 27 / 283** (remove dupes, move zeroes) | Both pointers move **left→right**; one reads, one writes | **Fast & slow** — that's sub-pattern 1.4, not this one |
...
- Comparing **backward** (`nums[i] == nums[i-1]`) keeps the *first* occurrence and skips the rest. Comparing forward (`nums[i] == nums[i+1]`) skips the first and keeps the last — which drops valid triplets like `[0,0,0]`.
- The inner skips must come **after** `l += 1; r -= 1`, not before. Before, and you never advance off the current hit.
...
NOT THIS
  ...
  both pointers move right            → fast & slow (1.4)
